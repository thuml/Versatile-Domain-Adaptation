...
import loss
import pre_process_visda as prep
...
def image_classification_val(loader, model, test_10crop=True):
    ...
    with torch.no_grad():
        if test_10crop:
            iter_test = [iter(loader['source_val'][i]) for i in range(10)]
            for i in range(len(loader['source_val'][0])):
...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        cov_matrix_t_temp = target_softmax_out_temp.mul(target_entropy_weight.view(-1,1)).transpose(1,0).mm(target_softmax_out_temp)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--net', type=str, default='ResNet101', choices=["ResNet18", "ResNet34", "ResNet50", "ResNet101", "ResNet152", "VGG11", "VGG13", "VGG16", "VGG19", "VGG11BN", "VGG13BN", "VGG16BN", "VGG19BN", "AlexNet"])
    parser.add_argument('--dset', type=str, default='visda', choices=['office', 'image-clef', 'visda', 'office-home'], help="The dataset or source dataset used")
    parser.add_argument('--s_dset_path', type=str, default='../data/visda-2017/train_list.txt', help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='../data/visda-2017/validation_list.txt', help="The target dataset path list")
    ...
    parser.add_argument('--output_dir', type=str, default='visda_temp3.0', help="output directory of our model (in ../snapshot directory)")
    ...
    config["output_path"] = "snapshot-eccv/" + task_name
